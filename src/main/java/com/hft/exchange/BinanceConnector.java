...
public class BinanceConnector {
    ...
    public BinanceConnector(List<String> symbols) {
        ...
        for (String s : symbols) {
            SymbolMapper.register(s.toLowerCase());
        }
    }
    ...
    public synchronized void connect() {
        ...
        try {
            ...
            for (int i = 0; i < symbols.size(); i++) {
                ...
                streams.append(symbols.get(i).toLowerCase().replace("/", "")).append("@trade");
            }
    ...
    private void processMessage(String message) {
        try {
            ...
            if (!json.has("s") || !json.has("p") || !json.has("q") || !json.has("T")) return;
            ...
            boolean isBuyerMaker = json.has("m") && json.get("m").getAsBoolean();
